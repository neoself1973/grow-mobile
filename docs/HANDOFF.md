...
- **撮影方法（人間の申告・2026-09-09）**: **QuickTime の「新規ムービー収録」で、カメラに iPhone を選んで収録**＝**実機の画面**。∴ README の「iOS は実機動作を動画で提示」（正本 §0.10 の確定文言）と事実が一致する。
- **検分（機械）**: 8/20/35/48 秒の各フレームを抽出して内容を確認。解像度 1080×1920 は iPhone 15 の実画面（1179×2556）ともシミュレータ（1206×2622）とも異なる＝**収録・書き出しの過程で切り出しと縮小がかかっている**（ステータスバーが映っていないのも同じ理由。QuickTime 経由の収録として整合する）。入力されている本文は**漢字かな交じりの自然な日本語**で、9/7 のシミュレータ録画（自動入力のためかな書き）とは明確に別物。
