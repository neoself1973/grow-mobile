...
| `@react-native-async-storage/async-storage` | 書きかけの積み上げの下書き |
...
## いまできること（2026-09-09 公開時点）

- 登録・ログイン・オンボーディング（5項目）
- **対話→結論の一周**（今日の積み上げを書く → Grow が読む → 深掘りの問答 → 確認 → 結論・明日の一手）。結論は Grow日あたり1回
- ダッシュボードの三層の数字（Web と同じ3つの問い合わせ・同じ定義）
- 書きかけの積み上げのオフライン下書き（端末内に残り、保存が成功したら消える）
- 短期レポートの生成の引き金（ダッシュボード到達時に1回。**閲覧は範囲外**）

**範囲外**: 設定・勝ち筋レポート（`/insights`）・短期レポートの閲覧・お知らせポップアップ・ホーム画面案内（PWA 固有）。範囲外の画面への導線も持ちません。
配布の形は上の「触れるもの（配布物）」の節のとおりです。
